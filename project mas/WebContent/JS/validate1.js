...
if(checkFirstName()& checkLastName()& checkRadio()& checkDob()& checkEmailId()&checkPhone()&checkCity()&checkPassword()&checkLocation()&setoptions(chosen))
{		
 flag=true;
}
...
{
	var var1 = document.frmOne.loc;
	var1.options.length = 0;
	if(chosen=="select")
		{
		var1.options[var1.options.length] = new Option('please select any one of the options','select');
		
		}
	if(chosen=="Delhi")
		{
		var1.options[var1.options.length]=new Option('noida','noida');
		var1.options[var1.options.length]= new Option('agra','agra');
		
		}
	
	if(chosen=="Pune")
		{
		var1.options[var1.options.length]=new Option('pune','pune');
		
		}
	if(chosen=="Trivandrum")
		{
		var1.options[var1.options.length]=new Option('tvm','tvm');
		}
}
